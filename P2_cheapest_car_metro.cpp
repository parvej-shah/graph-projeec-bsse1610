...
public:
    ...
    pair<vector<int>, double> solve(int start, int end) {
        ...
        while (!pq.empty()) {
            ...
            for (const Edge& e : nodes[current.node].edges) {
                if (e.type != "road" && e.type != "metro") continue;
    ...
    void printSolution(const Point& source, const Point& dest) {
        ...
        const vector<Node>& nodes = graph.getNodes();
        double costWithWalk = totalCost;  // walk is ৳0
        cout << "Total Cost: ৳" << fixed << setprecision(2) << costWithWalk << endl;
        ...
        generateKML(kmlPoints, "P2_route.kml");
        cout << "KML file generated: P2_route.kml" << endl;
